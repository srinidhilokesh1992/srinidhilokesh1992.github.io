      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
        ...
        <nav className={`hidden items-center gap-8 text-[10px] font-medium uppercase tracking-[0.22em] md:flex ${navTextClass}`}>
          {navConfig.map((item) => (
            <Link
              key={item.label}
              href={hrefFor(item)}
              className={`transition ${isActive(item) ? headingText + " font-semibold" : ""}`}
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          <button
            ...
            className={`inline-flex items-center justify-center rounded-full border px-3 py-2 text-[9px] font-medium uppercase tracking-[0.2em] transition ${iconButtonClass}`}
          >
            {isDark ? "Light" : "Dark"}
          </button>
          <Link
            href={isHome ? "#contact" : "/#contact"}
            className={`inline-flex items-center justify-center rounded-full px-5 py-2.5 text-[10px] font-medium uppercase tracking-[0.22em] transition ${contactButtonClass}`}
          >
            Contact
          </Link>
          <a
